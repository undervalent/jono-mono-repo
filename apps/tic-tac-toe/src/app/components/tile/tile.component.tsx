...
function useTile(value: Value, idx: Idx) {
  ...
  const hoveredIcon =
    currentPlayer === 'X' ? (
      <img src={XHoverIcon} alt="X hover" className={styles.hovered} />
    ) : (
      <img src={OHoverIcon} alt="O hover" className={styles.hovered} />
    );
  ...
  if (winningCondition && winningCondition.includes(idx)) {
    switch (value) {
      case 'X':
        icon = (
          <img src={XWinIcon} alt="X selected" className={styles.selected} />
        );
        break;
      case 'O':
        icon = (
          <img src={OWinIcon} alt="O selected" className={styles.selected} />
        );
        break;
      default:
        break;
